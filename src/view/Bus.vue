  ...
  <div class="bus">
      ...
      <form @submit.prevent="add">
        <table class="tabel-input">
          <tr>
            ...
            <th><label>Nama Bus:</label></th>
            <th><input type="text" v-model="form.bus" required /></th>
          </tr>
          <tr>
            <th><label>Merek Bus:</label></th>
    ...
    <div class="data">
      <table class="table table-striped tableData">
        <thead>
          <tr>
            <th class="th">Nama Bus</th>
            <th class="th">Merek Bus</th>
        ...
        <tbody>
          <tr v-for="bus in buss" :key="bus.id">
            <td class="td td1">{{ bus.bus }}</td>
            <td class="td td1">{{ bus.merk }}</td>
            <td class="td td1">{{ bus.tahun }}</td>
            <td class="td td1">{{ bus.bahanbakar }}</td>
            <td class="td td1">{{ bus.jenis }}</td>
            <td>
              <img
                class="td"
                :src="bus.image"
            ...
            <td class="td td1">
              <a href="#bus">
                <button class="edit" @click="edit(bus)">
                  <i class="fa-solid fa-pen-to-square"></i>
                </button>
              </a>
              <button class="del" @click="del(bus)">
...
export default {
  ...
  methods: {
    load() {
      axios
        .get(" http://localhost:3000/buss")
        .then((res) => {
          this.buss = res.data; //respon dari rest api dimasukan ke users
        })
        .catch((err) => {
          console.log(err);
        });
    ...
    add() {
      axios.post(" http://localhost:3000/buss/", this.form).then((res) => {
        this.load();
        this.form.bus = "";
        this.form.merk = "";
        this.form.tahun = "";
    ...
    edit(bus) {
      ...
      this.form.id = bus.id;
      this.form.bus = bus.bus;
      this.form.merk = bus.merk;
      this.form.tahun = bus.tahun;
      this.form.bahanbakar = bus.bahanbakar;
      this.form.jenis = bus.jenis;
      this.form.image = bus.image;
    },
    update(form) {
      return axios
        .put(" http://localhost:3000/buss/" + form.id, {
          bus: this.form.bus,
          merk: this.form.merk,
    ...
    del(bus) {
      axios.delete(" http://localhost:3000/buss/" + bus.id).then((res) => {
        this.load();
        let index = this.buss.indexOf(form.bus);
